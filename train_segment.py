...
import numpy as np
import cv2
from criterion import SegmentationMetric
...
criterion_segment = SegmentationMetric()
...
for epoch in range(opt.begin_epoch, opt.end_epoch):
    ...
    for i in range(0, lenNum):
        ...
        loss_seg = criterion_segment.meanIntersectionOverUnion(seg.cpu().detach().numpy(), mask.cpu().detach().numpy())
    ...
    if opt.need_test and epoch % opt.test_interval == 0 and epoch >= opt.test_interval:
        ...
        for i, testBatch in enumerate(testloader):
            ...
            testLoss = criterion_segment.meanIntersectionOverUnion(segTest.cpu().detach().numpy(), maskTest.cpu().detach().numpy())
